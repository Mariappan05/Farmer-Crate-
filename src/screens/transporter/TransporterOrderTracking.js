 ...
 * Animated order tracking for transporter view.
 *
 * Features:
 ...
 *   - Farmer / Customer / Delivery person info cards
  ...
  return (
    <View style={trackStyles.stepRow}>
      <View style={trackStyles.stepLeft}>
        ...
        >
          {stage.mcIcon ? (
            <MaterialCommunityIcons name={stage.mcIcon} size={18} color={isCompleted ? '#fff' : '#bbb'} />
          ) : (
...
const TransporterOrderTracking = ({ navigation, route }) => {
  ...
  const getNextStatus = () => {
    ...
    if (s === 'assigned' || s === 'confirmed') return 'SHIPPED';
  ...
  const handleUpdateStatus = async () => {
    ...
    Alert.alert('Update Status', `Mark order as ${next.replace(/_/g, ' ')}?`, [
      { text: 'Cancel', style: 'cancel' },
      {
        ...
        onPress: async () => {
          ...
          try {
            ...
            await api.put(`/transporters/orders/${id}/status`, { status: next });
            fetchOrder(true);
            Alert.alert('Success', `Order marked as ${next.replace(/_/g, ' ')}`);
          } catch (e) {
  ...
  return (
    <View style={[trackStyles.container, { paddingTop: insets.top }]}>
      ...
      <ScrollView
        ...
        refreshControl={
          <RefreshControl refreshing={refreshing} onRefresh={() => { setRefreshing(true); fetchOrder(true); }} colors={['#1B5E20']} tintColor="#1B5E20" />
        }
            ...
            {items.map((item, idx) => {
              ...
              return (
                <View key={idx} style={trackStyles.productRow}>
                  {img ? (
                    <Image source={{ uri: optimizeImageUrl(img, { width: 64, height: 64 }) }} style={trackStyles.productImg} />
                  ) : (
...
const trackStyles = StyleSheet.create({
  ...
  updateStatusBtn: {
    ...
    marginBottom: 16,
    ...Platform.select({
      ios: { shadowColor: '#1B5E20', shadowOffset: { width: 0, height: 3 }, shadowOpacity: 0.25, shadowRadius: 6 },
      android: { elevation: 4 },
    }),
  },
  ...
  actionBtnText: { fontSize: 15, fontWeight: '600', color: '#1B5E20' },
